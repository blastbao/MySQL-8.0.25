...
// [作用]
// mtr 保证了事件内部操纵的原子性。
// 当用户执行更新操作时，会更新数据页、同时写 redo log，而 mtr 正是 redo log 的载体，存在每个毗连会话的私有变量中。
// 当 mtr 提交时，会将其本地 redo log 拷贝到全局的 log_buffer 中，为了保证 redo log 的有序性，必要加锁来访问 log_buffer ，
// 在这个锁保护下，除了要将 mtr 本地日志拷贝到全局 log_buffer ，还必要将数据页加入了 flush_list ，供后台线程刷脏，
// 辅助数据库检查点一连往前推进，所以这个锁在旧版本的 MySQL 中竞争非常激烈。
...
// 1、the fix rules
//
//   介绍第一条规则之前，我们有必要相识下 MySQL 中的 latch 的概念，在 MySQL 中，latch 是一种轻量级的锁，
//   与 lock 不同，它锁定的时间特别短，在 innodb 中，latch 又可以分为 mutex(互斥量) 和 rwlock（读写锁）2 种，
//   它的目的在于保证并发线程操纵临界资源的准确性。
//
//   理解了 latch 的概念，我们看看 the fix rule 规则：
//      - 修改一个数据页，必需获得这个数据页的 x-latch ；
//      - 访问一个数据页，必需获得 s-latch 或者 x-latch ；
//      - 持有该页的 latch 直到修改、访问该页的操纵完成才释放；
//
// 2、WAL
//   WAL 是 Innodb 存储引擎支持崩溃规复的根本，也就是持久化一个数据页之前，必需将内存中相应的日志页先长期化
//
// 3、force-log-at-commit
//   在事件提交时，其产生的所有 MTR 日志都要刷到持久化设备中，从而保证崩溃规复的逻辑。
//
